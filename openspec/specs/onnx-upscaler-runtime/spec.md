# Capability Spec: onnx-upscaler-runtime

## Purpose

Provides an `OnnxImageUpscaler` that runs super-resolution inference using the ONNX Runtime CPU execution provider, plugging into the `ml-runtime` selection seam so the app can upscale manga pages with a real ML model without requiring a GPU.
## Requirements
