...
- **拡張**: `add-local-video-playback` が定義した `MediaSession` sealed hierarchy に **`PageSession`** バリアントを追加（読書位置 / ページめくり）。v0.2 の書籍 / 漫画と共有する最小 API で着地。
...
- `media-session`: `add-local-video-playback` で定義された sealed `MediaSession` に `PageSession` バリアントを追加し、読書位置（ページ番号 / スクロール率）を `MediaPosition` 互換で表現できるよう要件を拡張。
...
- `app/lib/core/novel/page_session.dart` — `MediaSession` の `PageSession` バリアント
...
- [`app/lib/core/storage/database.dart`](../../../app/lib/core/storage/database.dart) — `add-local-video-playback` が定義する `@DriftDatabase` に 4 テーブルと対応 DAO を追加（drift スキーマ v1 のままで OK、両 change がリリース前なので破壊的扱いではない）
- [`app/lib/core/media/media_session.dart`](../../../app/lib/core/media/media_session.dart) — sealed hierarchy に `PageSession` を足す
- [`app/lib/core/media/models.dart`](../../../app/lib/core/media/models.dart) — `MediaPosition` を「ms または ページ位置」の sealed 互換に拡張（`MediaPosition.byteOffset` を導入せず、`pageIndex` / `scrollFraction` の optional フィールド追加にとどめる方針は design で決定）
