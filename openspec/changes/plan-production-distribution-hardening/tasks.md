...
- [ ] 1.2 macOS notarization 導入（design.md D2）の要否と導入タイミングを確定する。
- [ ] 1.3 Android 本番 keystore 移行方針（design.md D3）と keystore バックアップ保管者・保管場所を確定する。
- [ ] 1.4 横断的な secrets 一覧・CI 権限・ローテーション責任（design.md D4）を確定し、`docs/release.md` に追記する下書きを用意する。

## 2. 後続 OpenSpec change への分割

- [ ] 2.1 iOS signing 実装用の OpenSpec change 名・スコープ案を確定する（1.1 の決定を前提とする）。
- [ ] 2.2 macOS notarization 実装用の OpenSpec change 名・スコープ案を確定する（1.2 で「導入する」と決まった場合のみ）。
- [ ] 2.3 Android production signing 実装用の OpenSpec change 名・スコープ案を確定する（1.3 の決定を前提とする）。
- [ ] 2.4 上記の分割案を GitHub Issue #49 の子 Issue またはコメントとして記録する（GitHub 管理チャットに引き継ぎ）。
