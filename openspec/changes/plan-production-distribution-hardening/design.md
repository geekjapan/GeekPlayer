...
- D1〜D3 が必要とする secrets（iOS: 配布証明書 + プロビジョニングプロファイル、macOS: Developer ID 証明書 + notarization API key、Android: keystore + パスワード）はすべて GitHub Actions の Environment Secrets（例: `release` 環境。`.github/workflows/release-artifacts.yaml` のリリースジョブのみが参照可能な最小権限）に格納する方針とする。
