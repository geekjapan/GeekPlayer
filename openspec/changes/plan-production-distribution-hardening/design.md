...
| 選択肢 | 概要 | Trade-off |
| --- | --- | --- |
| A. Apple Developer Program（個人/組織, 年額 $99）+ Ad Hoc 配布 | UDID 登録済み端末（プログラムあたり最大100台/年）向けに配布用 `.ipa` を署名 | ✅ ADR-0006 と整合。⚠️ 端末登録の運用コストが利用者拡大時にボトルネック。年次の証明書更新が必要。 |
...
**現時点の判断**: A を第一候補とするが、**年会費 $99 の負担主体（プロジェクト予算 or 個人負担）と、Ad Hoc 端末登録上限にどう対応するか（100台で足りない場合の運用）は人間/プロダクトオーナーの判断が必要**なため、本 change では「A を前提に後続 change を設計する」ことのみ決定し、費用負担の可否は open question として残す。
...
- D1〜D3 が必要とする secrets（iOS: 配布証明書 + プロビジョニングプロファイル、macOS: Developer ID 証明書 + notarization API key、Android: keystore + パスワード）はすべて GitHub Actions の repository secrets（`.github/workflows/release-artifacts.yaml` が参照可能な最小権限）に格納する方針とする。
- 証明書の有効期限（Apple 証明書は最長1年）到来前に更新する担当・手順を `docs/release.md` に追記することを後続 change のタスクに含める。
- 秘密情報をリポジトリに直書きしないという `rules/security.md` の最低線は、後続の実装 change でも継続して守る前提とする。
...
- [Risk] iOS Ad Hoc の端末登録上限（100台/年）が OSS 配布の想定利用者数を超える可能性 → Mitigation: 実際に上限に到達した場合の代替案（EU 代替マーケットプレイス、招待制配布等）を open question として残し、後続 change 着手前に人間判断を仰ぐ。
...
2. iOS: Ad Hoc 配布の端末登録上限（100台/年）に到達した場合の代替方針を今から決めておくか、到達時に判断するか。
