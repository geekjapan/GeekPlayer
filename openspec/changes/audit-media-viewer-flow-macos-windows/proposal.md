...
macOS / Windows での実機確認により、複数のメディア／ビューア機能面で「開いたあと操作方法が分かりにくい、または操作対象が一時的に使えないように見える」状態が確認されている（issue #50）。動画には `Icons.arrow_back` と `Navigator.of(context).maybePop()` による戻る導線が実装済みだが、自動非表示オーバーレイの中にあるためデスクトップ上で見失いやすい。漫画/コミックにも AppBar と戻る導線は実装済みだが、コントロール表示状態に依存するため、ページ送り領域とともに利用不能に見える状態を再現できる。issue #43 由来のバッチ4（`ui-phase-2-batch-4-remove-placeholders-policy-debug`）は novel 面の表示専用 artifact 除去に限定してスコープを閉じ、issue #50（本件）と #51（ホーム IA）を明示的に Non-goals として先送りにした。本 change はその続きとして #50 に着手する。
...
- **修正前のデスクトップ再現証拠**: macOS/Windows のデスクトップ入力で、動画を開いて 3 秒待機した後に戻るボタンが見えず、映像面クリックでオーバーレイを再表示して初めて `maybePop` を実行できることを確認する。漫画ではコントロールを非表示にした後、AppBar（戻る導線を含む）とページ送りタップ領域が同時に無効になり、画面中央のクリックは表示復帰だけになることを確認し、再現手順・観測結果・`file_path:line_number` の根拠を `design.md` に残す。既存のコントロールを「存在しない」と記録せず、見えない／条件付きで無効になる問題として扱う。
