...
- drift テーブル `novel_works`, `novel_episodes`, `novel_bookmarks`, `site_consents`（`add-online-novel-library` 提供。本 change はテーブルを追加しない）
- `RateLimiter` ユーティリティ
...
- 縦スクロールのリーダー画面で、フォントサイズ / 行間 / 明暗テーマ / 栞 / 前後話遷移を
  提供する
- [ADR-0003](../../../docs/adr/0003-narou-content-fetch-policy.md) の規範（API: 1 req/sec / 並列 2、本文ページ: 1 req/sec / 並列 1、`*.syosetu.com` 共通バケット、指数バックオフ最大 6 回 / max 5 分、User-Agent 規範、`robots.txt` 尊重）を遵守する
...
5. 「はい」で `SiteConsentRepository.grant(Site.noc)` を呼び、ダイアログを閉じて R18 機能を解放
6. 「いいえ」または閉じる操作でダイアログをキャンセル扱い、R18 機能ロックを継続

同意は **3 サイト（noctune / midnight / moonlight）で 1 つのフラグ** にする
（API が共通の `novel18api` で内部的に同系統扱いだから）。`add-online-novel-library`
の `Site` enum に既にある `Site.noc` を統合キーとして使う（追加の enum 値は不要）。

`Settings > オンライン小説` 画面に「年齢確認をやり直す」項目を追加。タップで
`SiteConsentRepository.revoke(Site.noc)` を呼ぶ。
...
(`light` / `sepia` / `dark`) の値オブジェクト。永続化は `add-app-settings` の
`app_settings(key TEXT PK, value TEXT)` テーブルを使う。key の名前空間は
`novel.reader.fontSize` / `novel.reader.lineHeight` / `novel.reader.colorScheme`
で統一（カクヨムリーダーも同 key を購読）。本 change では新規テーブルを足さない。

栞は `add-online-novel-library` の `novel_bookmarks` テーブルを使う:
`(site, externalId, episodeIndex, scrollFraction, updatedAt)` を保存し、scrollOffset
(pixel) は scrollFraction (0.0〜1.0) に変換して保存する（フォント変更でレイアウトが
変わってもページ位置を保てる）。
...
1. `LibraryRepository.add(work)` を呼ぶ（`add-online-novel-library` 提供）
2. `LibraryRepository` 側で全話のメタデータ取得 → 本文ダウンロードジョブを起動
3. 本 change の `NarouEpisodeFetcher` がジョブから呼ばれて 1 話ずつ本文取得
4. レート制限 1 req/sec を守るので、100 話 = 約 100 秒
5. 進捗は `LibraryRepository` 提供の `Stream<DownloadProgress>` で UI に流す
...
- **Q-D1**: なろう連載作品の本文取得の正規エンドポイント
  - **[ADR-0003](../../../docs/adr/0003-narou-content-fetch-policy.md) で決定済み**:
    なろう公式 API は本文を返さないため、HTML パースで取得する（カクヨムと同じ
    責任あるスクレイピング規範を適用）
  - 各話本文取得は `https://ncode.syosetu.com/<ncode>/<episode>/` (一般) /
    `https://novel18.syosetu.com/<ncode>/<episode>/` (R18) の HTML を `html`
    パッケージでパース。短編は単一ページ、連載はエピソード番号ごと別ページ
