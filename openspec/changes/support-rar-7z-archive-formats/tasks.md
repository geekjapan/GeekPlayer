...
- [ ] 1.4 `libarchive` の RAR5 対応状況とライセンス互換性を評価し、結果をフォローアップ GitHub Issue #69 に追記する（本 change では RAR/CBR を実装しない）。
...
- [ ] 4.1 `.rar`/`.cbr` 拡張子を検出した際に、汎用 `UnsupportedFormatError` ではなく「現時点で未対応（Issue #69 で検討中）」であることが分かる専用メッセージ/エラーコードを追加する。
...
- [ ] 6.3 RAR/CBR 完全対応のフォローアップ GitHub Issue #69 に評価結果を記載し、Issue #52 から #69 へのリンクがあることを確認する。
