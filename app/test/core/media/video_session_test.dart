...
void main() {
  group('VideoSession (streams-only test seam)', () {
    ...
    test('positionStream emits transformed MediaPosition on engine event',
        () async {
    ...
    test('playStateStream emits playing when engine reports playing=true',
        () async {
    ...
    test('playStateStream emits paused when engine reports playing=false',
        () async {
      final Future<MediaPlayState> first = session.playStateStream.first;
      playing.add(false);
      final MediaPlayState s = await first;
    ...
    test('completed=true overrides to ended even when playing was true',
        () async {
    ...
    test('durationStream emits null for Duration.zero, value otherwise',
        () async {
      final List<Duration?> values = <Duration?>[];
      final StreamSubscription<Duration?> sub =
          session.durationStream.listen(values.add);
    ...
    test('dispose() completes streams and subsequent commands throw',
        () async {
      await session.dispose();
    ...
    test('setSpeed updates speed getter even with no underlying player',
        () async {
