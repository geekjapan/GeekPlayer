...
void main() {
  ...
  test(
    'mutate updates state synchronously and persists after debounce',
    () async {
      final AppDatabase db = _freshDb();
      addTearDown(db.close);
      final ProviderContainer c = _container(db);
  ...
  test('rapid mutations coalesce into a single write per key', () async {
    ...
    expect(c.read(appSettingsProvider).value!.novelFontSizeSp, 22.0);
  });

  test(
    'flush() persists immediately without waiting for the debounce',
    () async {
      final AppDatabase db = _freshDb();
      addTearDown(db.close);
      final ProviderContainer c = _container(db);
      ...
      c
          .read(appSettingsProvider.notifier)
          .mutate((AppSettings s) => s.copyWith(subtitlesByDefault: true));
