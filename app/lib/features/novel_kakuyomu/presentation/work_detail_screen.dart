  ...
  void _load() {
    setState(() {
      _pending = ref.read(kakuyomuNovelRepositoryProvider.future).then((
        ...
        return repo.fetchWorkDetail(widget.workId)
            as Future<KakuyomuWorkDetail>;
      });
    });
  }

  Future<void> _addToLibrary(KakuyomuWorkDetail detail) async {
    final KakuyomuNovelRepository? repo = await ref.read(
      kakuyomuNovelRepositoryProvider.future,
    );
    if (repo == null) return;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: FutureBuilder<KakuyomuWorkDetail>(
        future: _pending,
        builder:
            (BuildContext context, AsyncSnapshot<KakuyomuWorkDetail> snap) {
