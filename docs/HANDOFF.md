...
最終更新: 2026-06-17（**v0.1.1 リリース済み、デザインシステム基盤 + UI correctness sweep バッチ1 完了。Phase 2 残項目が次の作業**）。
...
この Windows マシンには **Flutter/Dart がインストールされていません**。
...
以下の UI correctness / a11y 修正が残っています。新しい OpenSpec change を起こして
小バッチで進めてください（ローカル Flutter 無しのため 1 バッチ＝1 CI サイクル）。

| # | 内容 | 難度 | 備考 |
|---|---|---|---|
| ④ | 生エラー文字列 `$e` → ユーザ向けメッセージ | 中 | kakuyomu ranking/latest_feed, narou reader, manga/book home 等。`ErrorMessages.localize` 的な contract 設計が必要 |
| ⑥ | ISO-8601 日付を人間が読める形式に整形 | 低〜中 | kakuyomu の日付表示、共通 `formatNovelDate` util 新設 |
| ⑦ | 永久無効プレースホルダボタン + `policyVersion` デバッグ文言除去 | 中 | `online-novel-library` spec と絡む。慎重に |
| ⑨ | 破壊的操作ボタン/アイコンに `colorScheme.error` 適用 | 低 | 設定画面の確認ダイアログ 4 箇所 |
| ⑩ | インラインライセンス/notice リンクを `TextButton.icon` (48dp) に | 低 | タッチターゲット確保 |
| ⑪ | 設定セクションヘッダの余白 + M3 section-label スタイル | 低 | 体裁改善 |

推奨バッチ分割:
- **バッチ2**: ⑨⑩⑪（a11y/体裁、比較的独立、低リスク）
- **バッチ3**: ④⑥（正しさ、contract 設計が要る）
- **バッチ4**: ⑦（spec 絡み、単独で丁寧に）
...
- **Android file-provider**: ✅ 解決済み（#45）。`install handoff` は `open_filex` 同梱の `FileProvider`（authority `${applicationId}.fileProvider.com.crazecoder.openfile`）経由で `content://` URI を生成しており、**自前の `<provider>` は authority 衝突回避のため意図的に宣言しない**（`add-android-apk-install-handoff` design D2/D3）。`AndroidManifest.xml` は `REQUEST_INSTALL_PACKAGES` と install-intent `<queries>` を宣言済み、`update_installer.dart` は Android のみ `OpenFilex.open` を呼ぶ。実機検証済み（2026-06-07, task 4.6b: `ACTION_VIEW` + `content://` + `application/vnd.android.package-archive` → system installer、`FileUriExposedException` なし）。今後 `<provider>` を追加しないこと（manifest merge 衝突を招く）。
...
- `.claude/`, `.codex/`, `.pi/` の `skills/` — 変更時は 3 ハーネス同時更新
- `openspec/specs/` — `/opsx:archive` 経由でのみ更新、手動編集禁止
- 既存 ADR — 方針変更時は superseded な新 ADR を立てる
