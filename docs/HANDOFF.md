...
- **OpenSpec archive**: v0.1 の 8 changes（`archive/2026-05-27-*`）に加え、v0.2 の 9 changes（`archive/2026-06-03-*`）もすべて archive 済み。
  ...
  - v0.2: prepare-v0-2-foundation / add-english-localization / add-pdf-epub-reader / add-manga-zip-viewer / add-media-library / expand-ci-and-platforms / add-auto-update / add-ml-runtime-abstraction / expand-auto-update-delivery
- **capabilities**: 32 capability が `openspec/specs/` に確定（手動編集禁止、`/opsx:archive` 経由でのみ更新）。`openspec validate --specs --strict` は 32/32 pass。
...
- **v1.0 AI 高画質化の具体バックエンド**（下記）— `add-ml-runtime-abstraction` の seam に CoreML/NNAPI/ONNX/TensorRT の concrete backend と Real-ESRGAN/waifu2x モデルを載せる。

完了済みで候補から外れた項目（履歴）: `add-video-library`/`add-audio-library`（→ `add-media-library`）、`add-platform-linux`（→ `expand-ci-and-platforms` の Linux scaffolding + CI smoke）、`add-auto-update`（バナー版）+ `expand-auto-update-delivery`（OS 別ダウンロード/handoff）、`setup-ci-macos-windows`（→ `expand-ci-and-platforms`）、`add-ml-runtime-abstraction`（`core/ml/` 抽象 + passthrough）。
...
- **auto-update の Android install handoff** — `expand-auto-update-delivery` は `launchUrl(Uri.file(path))`
  で OS へ handoff するが、Android では APK install に `FileProvider` の AndroidManifest 宣言が未追加のため
  ランタイムで失敗する。Android で実 install を有効化する際に file-provider を足すこと（macOS/Windows/Linux は動作、iOS は対象外）。
...
2. **v1.0 AI 高画質化の concrete backend** — `add-ml-runtime-abstraction` の `ImageUpscaler` seam（`core/ml/`、passthrough default 実装済み）に CoreML/NNAPI/ONNX/TensorRT の実バックエンドと Real-ESRGAN/waifu2x モデルを段階導入（roadmap §v1.0）。
3. **auto-update の Android file-provider 対応** — `expand-auto-update-delivery` は実装済みだが、Android で `file://` install handoff に AndroidManifest の file-provider 宣言が必要（既知の残課題、§7 参照）。
